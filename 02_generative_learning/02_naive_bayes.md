...
#### Python Example: Parameter Estimation

Below is Python code to compute $\phi_{j|y=1}$, $\phi_{j|y=0}$, and $\phi_y$ for a binary feature matrix X and label vector y:
...
# Example usage with the tiny dataset from above:
X = np.array([
    [1, 1, 0],
    [1, 0, 1],
    [0, 1, 0],
...
Below is Python code to compute the posterior probability $p(y=1|x)$ for a new sample x, given estimated parameters:
...
# Example: predict for x = [1, 0, 1] ("buy" and "now" present)
x_new = np.array([1, 0, 1])
p_y1_given_x = predict_naive_bayes(x_new, phi_j_y1, phi_j_y0, phi_y)
print("P(y=1|x_new):", p_y1_given_x)
```

#### Python Example: Laplace Smoothing (Bernoulli Naive Bayes)

Below is Python code to compute Laplace-smoothed estimates for $\phi_{j|y=1}$ and $\phi_{j|y=0}$:

```python
def laplace_smoothing_bernoulli(X, y):
    n_samples, n_features = X.shape
    # Add 1 to numerator, 2 to denominator (binary features)
...
#### Python Example: Multinomial Naive Bayes Parameter Estimation and Laplace Smoothing

Below is Python code to compute $\phi_{k|y=1}$ and $\phi_{k|y=0}$ for the multinomial event model, with Laplace smoothing:

```python
def multinomial_naive_bayes_params(X, y, vocab_size):
    # X: list of arrays, each array is the sequence of word indices for a document
...
# Example usage:
# Suppose vocab_size = 3, and X = [[0,1,0], [2,0], [1,1], [2]]; y = [1,1,0,0]
vocab_size = 3
