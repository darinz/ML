...
4. **Variance calculation:** 
   $$
   \operatorname{Var}(Y) = \operatorname{Var}(\sum_{i=1}^n X_i) = \sum_{i=1}^n \sum_{j=1}^n \operatorname{Cov}(X_i, X_j) = \mathbf{1}^T \Sigma \mathbf{1}
   $$
...
2. **Key formula:** For any matrix $A$ and Gaussian $X \sim \mathcal{N}(\mu, \Sigma)$:
   $$
   \mathbb{E}[X^T A X] = \operatorname{tr}(A \Sigma) + \mu^T A \mu
   $$

3. **Apply with $A = \Sigma^{-1}$:**
   $$
   \mathbb{E}[X^T \Sigma^{-1} X] = \operatorname{tr}(\Sigma^{-1} \Sigma) + \mu^T \Sigma^{-1} \mu
   $$

4. **Simplify:** $\operatorname{tr}(\Sigma^{-1} \Sigma) = \operatorname{tr}(I) = n$
